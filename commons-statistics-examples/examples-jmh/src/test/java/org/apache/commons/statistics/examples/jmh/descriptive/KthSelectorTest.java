...
class KthSelectorTest {
    ...
    void testPartitionDNF(double[] values, int[] indices) {
        assertPartition(values, indices, new KthSelector()::partitionDNF);
    }
    ...
    void testSortDNF(double[] values) {
        assertSort(values, new KthSelector()::sortDNF);
    }
    ...
    void testInsertionSort(double[] values) {
        ...
        assertSort(values, x -> KthSelector.insertionSort(x, 0, x.length, false));
        if (values.length < 2) {
            return;
        }
        // Check internal sort
        ...
        // check internal sort
        assertSort(values, x -> KthSelector.insertionSort(x, 1, x.length, true));
    }
    ...
    void testInsertionSort5(double[] values) {
        ...
        assertSort(data, x -> KthSelector.insertionSort5(x, 0, 1, 2, 3, 4));
    }
