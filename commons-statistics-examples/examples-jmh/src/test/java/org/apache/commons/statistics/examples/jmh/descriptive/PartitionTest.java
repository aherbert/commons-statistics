...
//
    ...
    void testPartitionSBM(double[] values, int[] indices) {
        assertPartition(values, indices, new Partition()::partitionSBM);
    }
//
//    @ParameterizedTest
//    @MethodSource(value = {"testPartition"})
...
//
    ...
    void testSortSBM(double[] values) {
        assertSort(values, new Partition(PivotingStrategy.DYNAMIC, 3)::sortSBM);
    }
//
//    @ParameterizedTest
//    @MethodSource(value = {"testSort"})
...
//
    ...
    void testInsertionSort(double[] values) {
        ...
        assertSort(values, x -> Partition.insertionSort(x, 0, x.length - 1, false));
        if (values.length < 2) {
            return;
        }
        // Check internal sort
        ...
        // check internal sort
        assertSort(values, x -> Partition.insertionSort(x, 1, x.length - 1, true));
    }
//
//    @ParameterizedTest
//    @MethodSource(value = {"testSort"})
...
//        assertSort(data, x -> Partition.insertionSort5(x, 0, 1, 2, 3, 4));
//    }
