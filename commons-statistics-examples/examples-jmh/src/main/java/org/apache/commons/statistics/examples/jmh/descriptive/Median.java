...
public final class Median {
     ...
     * @return the median
     */
    public double evaluateIBM(double[] values) {
        // Floating-point data handling
        final DoubleDataTransformer t = transformer.get();
        final double[] x = t.preProcess(values);
        final int n = t.size();
        ...
        if ((n & 0x1) == 0x1) {
            if (m < len) {
                final int[] k = new int[] {m};
                partition.partitionIBM(x, len, k, 1);
                t.postProcess(x, k, 1);
            } else {
                t.postProcess(x, null, 0);
            }
        ...
        if (m - 1 < len) {
            final int kn = m < len ? 2 : 1;
            partition.partitionIBM(x, len, k, kn);
            t.postProcess(x, k, kn);
        } else {
            t.postProcess(x, null, 0);
        }
     ...
     * @return the median
     */
    public double evaluateIKBM(double[] values) {
        // Floating-point data handling
        final DoubleDataTransformer t = transformer.get();
        final double[] x = t.preProcess(values);
        final int n = t.size();
        ...
        if ((n & 0x1) == 0x1) {
            if (m < len) {
                final int[] k = new int[] {m};
                partition.partitionIKBM(x, len, k, 1);
                t.postProcess(x, k, 1);
            } else {
                t.postProcess(x, null, 0);
            }
        ...
        if (m - 1 < len) {
            final int kn = m < len ? 2 : 1;
            partition.partitionIKBM(x, len, k, kn);
            t.postProcess(x, k, kn);
        } else {
            t.postProcess(x, null, 0);
        }
