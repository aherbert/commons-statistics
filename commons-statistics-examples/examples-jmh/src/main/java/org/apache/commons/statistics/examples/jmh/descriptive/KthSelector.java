 ...
 * into the cache can be increased using 2 * i + 2 without overflow.
 *
...
class KthSelector {
    ...
    // TODO - rewrite methods to use these generic interfaces
     ...
     */
    private interface SPEPartitionFunction {
        /**
         * Partition an array slice around a single pivot. Partitioning exchanges array
         * elements such that all elements smaller than pivot are before it and all
         * elements larger than pivot are after it.
     ...
     */
    private interface DPPartitionFunction {
        /**
    ...
    double selectSP(double[] data, int k, double[] kp1) {
        ...
        while (end - begin > minSelectSize) {
            ...
            final int pivot = partitionSP(data, begin, end, pivotingStrategy.pivotIndex(data, begin, end));
            if (k == pivot) {
    ...
    double selectSPN(double[] data, int k, double[] kp1) {
        ...
        while (end - begin > minSelectSize) {
            // Select a pivot and partition data array around it
            final int pivot = partitionSPN(data, begin, end, pivotingStrategy.pivotIndex(data, begin, end));
            if (k == pivot) {
                // The pivot was exactly the element we wanted
                if (data[k] == 0) {
    ...
    double selectSPH(double[] data, int[] pivotsHeap, int k, double[] kp1) {
        ...
        while (end - begin > minSelectSize) {
            ...
            if (node < heapLength && pivotsHeap[node] >= 0) {
                ...
                // Select a pivot and partition data array around it
                pivot = partitionSP(data, begin, end, pivotingStrategy.pivotIndex(data, begin, end));
                if (node < heapLength) {
    ...
    private static double finalSelection(double[] data, int k, double[] kp1, int end) {
        if (kp1 != null) {
            ...
            // If this overflows (end == data.length && data.length == MAX_INTEGER)
            // then the for-loop is ignored.
            final int limit = Math.min(data.length, end + 1);
            double min = data[k + 1];
    ...
    // TODO:
    // Is it possible to consolidate these methods to use the interface:
    // partition(double[] data, int begin, int end, BitSet pivots, int k)
    ...
    private void partitionSP(double[] data, int begin, int end, BitSet pivots, int k) {
        ...
        while (end - begin > minSelectSize) {
            ...
            final int pivot = partitionSP(data, begin, end,
                pivotingStrategy.pivotIndex(data, begin, end));
            pivots.set(pivot);
            if (k == pivot) {
                // The pivot was exactly the element we wanted
                return;
     ...
     * order of signed zeros.
     *
     * <p>Requires a maximum of 10 float comparisons (n * (n-1) / 2). Assumes all indices
     * are valid.
     *
     * @param data Data array.
     * @param a Index.
     ...
     * @param e Index.
     */
    static void insertionSort5(double[] data, int a, int b, int c, int d, int e) {
        // Unrolled implementation of the loop method above with no bounds checks
        double v;
        if (data[b] < data[a]) {
            // swap(data, a, b)
            v = data[b];
    ...
    void partitionSBM(double[] data, int... k) {
        ...
        //
        // The strategy is abstracted into a data structure that can dynamically switch
        // between the two.
        // All k-indices are positive: duplicates can be flagged by bit flipping to negative.
        // The processing loop then ignores any k outside the min/max range, and
        // partitions each k between the range provided.
        // Note: Strategy 1 will destructively modify k via a sort.

        // TODO:
        // An initial pass finds the min/max k.
        // A second pass identifies the unique pivots.
        // Duplicates are set in the array k as negative.
    ...
    private void partitionSBM(double[] data, int begin, int end, BitSet pivots, int k) {
        ...
        while (end - begin > minSelectSize) {
            ...
            final int from = partitionSBM(data, begin, end,
                pivotingStrategy.pivotIndex(data, begin, end), upper);
...
//     * @param k Indices.
//     */
//    void partitionBM2(double[] data, int... k) {
//        final int n = k.length;
//        if (n <= 1) {
//            if (n == 1) {
//                select(data, k[0], null);
//            }
//            return;
//        }
//        // Multiple pivots
...
//        final int length = sortNaN(data);
//
//        final IndexSet pivots = new IndexSet(length);
//
//        for (int i = 0; i < n; i++) {
//            final int kk = k[i];
//            if (kk >= length || pivots.get(kk)) {
...
//                }
//            }
//            partitionBM2(data, begin, end, pivots, kk);
//        }
//    }
//
//    /**
...
//     * @param k Index whose value in the array is of interest.
//     */
//    private void partitionBM2(double[] data, int begin, int end, IndexSet pivots, int k) {
//        // Find the unsorted range containing k
//        final int[] upper = {0};
//        while (end - begin > minSelectSize) {
//            // Select a value and partition data array around it
...
//    }
    ...
    private static void partitionDP5(double[] a, int left, int end, int[] bounds) {
        ...
        int p4 = p3 + sixth;
        int p5 = p4 + sixth;
        insertionSort5(a, p1, p2, p3, p4, p5);
     ...
     * }</pre>
     *
     * <p>Uses a Dutch-National-Flag quicksort partition method.
     *
     * @param data Values.
     * @param k Indices.
     */
    void partitionDNF(double[] data, int... k) {
        final int n = k.length;
        if (n <= 1) {
            if (n == 1) {
                selectSP(data, k[0], null);
        ...
        // Multiple pivots
        final BitSet pivots = new BitSet(data.length);
        // Handle NaN
        final int end = sortNaN(data);
        if (end < data.length) {
            setPivots(end, data.length, pivots);
        }

        for (int i = 0; i < n; i++) {
            partitionDNF(data, pivots, i == 0 && end == data.length, k[i]);
        }
    }
     ...
     * @param data Data array to use to find out the K<sup>th</sup> value.
     * @param pivots Cache of pivot points.
     * @param first Set to {@code true} if this is the first call.
     * @param k Index whose value in the array is of interest.
     */
    private void partitionDNF(double[] data, BitSet pivots, boolean first, int k) {
        // Find the unsorted range containing k
        int begin;
        int end;
        if (first) {
...
//        final int[] to = {0};
//        final int from = partitionBM(data, begin, end,
//            pivotingStrategy.pivotIndex(data, begin, end), to);
...
//    }
     ...
     * @param data Values.
     */
    void sortDNF(double[] data) {
        sortDNF(data, 0, sortNaN(data));
    }
     ...
     * @param end Upper bound (exclusive).
     */
    private void sortDNF(double[] data, int begin, int end) {
        if (end - begin <= 1) {
            return;
        }
        final int[] to = {0};
