...
import org.apache.commons.statistics.inference.KolmogorovSmirnovTest.Options;
...
class KolmogorovSmirnovTestTest {
    ...
    void testOptions() {
        ...
        Assertions.assertEquals(PValueMethod.AUTO, o.getPValueMethod());
        Assertions.assertEquals(false, o.isStrictInequality());
        final Options.Builder b = Options.builder();
        Assertions.assertThrows(NullPointerException.class, () -> b.setAlternative(null));
        Assertions.assertThrows(NullPointerException.class, () -> b.setPValueMethod(null));
        for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
            for (final PValueMethod m : PValueMethod.values()) {
                for (final boolean strict : new boolean[] {true, false}) {
                    final Options o1 = o.toBuilder().setAlternative(h).setPValueMethod(m)
                        .setStrictInequality(strict).build();
                    Assertions.assertEquals(h, o1.getAlternative());
                    Assertions.assertEquals(m, o1.getPValueMethod());
                    Assertions.assertEquals(strict, o1.isStrictInequality());
                    final Options o2 = o1.toBuilder().build();
                    Assertions.assertEquals(h, o2.getAlternative());
                    Assertions.assertEquals(m, o2.getPValueMethod());
                    Assertions.assertEquals(strict, o2.isStrictInequality());
                }
            }
        }
    }
    ...
    void testOneSampleThrows() {
        ...
        final AlternativeHypothesis alternative = AlternativeHypothesis.TWO_SIDED;
        testOneSampleThrows(x -> KolmogorovSmirnovTest.statistic(x, cdf, alternative));
        testOneSampleThrows(x -> KolmogorovSmirnovTest.test(x, cdf));
    }
    ...
    void testOneSample(double[] x, DoubleUnaryOperator cdf,
            ...
            PValueMethod method) {
        final Options.Builder b = Options.builder().setPValueMethod(method);
        int i = 0;
        ...
        for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
            ...
            final KolmogorovSmirnovTest.OneResult r =
                ...
                method == Options.defaults().getPValueMethod() ?
                KolmogorovSmirnovTest.test(x, cdf) :
                KolmogorovSmirnovTest.test(x, cdf, b.build());

            final double d = KolmogorovSmirnovTest.statistic(x, cdf, h);
            s[i] = d;
            if (i == 0) {
                sign = r.getSign();
            } else if (i == 1) {
    ...
    void testTwoSampleThrows() {
        final AlternativeHypothesis alternative = AlternativeHypothesis.TWO_SIDED;
        testTwoSampleThrows((x, y) -> KolmogorovSmirnovTest.statistic(x, y, alternative));
        testTwoSampleThrows((x, y) -> KolmogorovSmirnovTest.test(x, y));
    }
    ...
    void testTwoSample(double[] x, double[] y,
                       double[] statistic, double[] p, double eps,
                       PValueMethod method, boolean strict) {
        final Options.Builder b = Options.builder()
                .setPValueMethod(method)
                .setStrictInequality(strict);
        double[] sx = x.clone();
        double[] sy = y.clone();
        Arrays.sort(sx);
        Arrays.sort(sy);
        final int tiesX = (int) Arrays.stream(x).filter(v -> Arrays.binarySearch(sy, v) >= 0).count();
        final int tiesY = (int) Arrays.stream(y).filter(v -> Arrays.binarySearch(sx, v) >= 0).count();
        int i = 0;
        final double[] s = new double[3];
        int sign = 0;
        for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
            ...
            final KolmogorovSmirnovTest.TwoResult r =
                h == Options.defaults().getAlternative() &&
                method == Options.defaults().getPValueMethod() &&
                strict == Options.defaults().isStrictInequality() ?
                KolmogorovSmirnovTest.test(x, y) :
                KolmogorovSmirnovTest.test(x, y, b.build());

            Assertions.assertEquals(tiesX, r.getTiesX(), "ties x");
            Assertions.assertEquals(tiesY, r.getTiesY(), "ties y");

            final double d = KolmogorovSmirnovTest.statistic(x, y, h);
            s[i] = d;
            if (i == 0) {
                sign = r.getSign();
            } else if (i == 1) {
    ...
    static Stream<Arguments> testTwoSample() {
        ...
        builder.add(Arguments.of(
            ...
            new double[] {0.10557708545324647, 0.05279395527073546, 1}, 1e-15, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
            ...
            new double[] {0.0462986609429517, 0.02314933691342261, 0.9304029304029304}, 1e-15, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
            ...
            new double[] {0.0030074360223340544, 0.43092369163123856, 0.0015037180115200998}, 1e-15, PValueMethod.EXACT, false));
        // Square input
        builder.add(Arguments.of(
                new double[] {0, 6, 9, 5, 2},
                new double[] {1, 8, 3, 4, 7},
                new double[] {0.2, 0.2, 0.2},
                new double[] {1, 0.83333333333333333, 0.83333333333333333}, 1e-15, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
                new double[] {6, 10, 11, 9, 8, 12, 19, 5, 14, 7},
                new double[] {3, 0, 2, 15, 13, 16, 4, 17, 18, 1},
                new double[] {0.5, 0.3, 0.5},
                new double[] {0.16782134274394334, 0.4195804195804196, 0.08391608391608392}, 1e-15, PValueMethod.EXACT, false));
        // Identical input
        builder.add(Arguments.of(
                new double[] {1, 2, 3},
                new double[] {1, 2, 3},
                new double[] {0, 0, 0},
                new double[] {1, 1, 1}, 0, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
                new double[] {1, 2},
                new double[] {1, 2},
                new double[] {0, 0, 0},
                new double[] {1, 1, 1}, 0, PValueMethod.EXACT, true));
        ...
        builder.add(Arguments.of(GAUSSIAN, GAUSSIAN2,
                ...
                new double[] {0.026098802349515924, 0.013049427737810542, 0.6597327117836644}, 1e-13, PValueMethod.AUTO, false));
        // Asymptotic
        // - compute en = n*m/(n+m)
        ...
        builder.add(Arguments.of(GAUSSIAN, GAUSSIAN2,
                new double[] {0.2023529411764706, 0.2023529411764706, 0.06},
                new double[] {0.028514899820936468, 0.013077034357100743, 0.6548386977618049}, 1e-13, PValueMethod.ASYMPTOTIC, false));
        // Auto chooses asymptotic due to ties.
        ...
        builder.add(Arguments.of(
                ...
                new double[] {0.9444444444444444, 0.48567178524771226, 0.4856717852477124}, 1e-15, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
                new double[] {1, 2, 2, 2, 3},
                new double[] {2, 2, 4},
                new double[] {1 / 3.0, 1 / 3.0, 0},
                new double[] {0.9444444444444444, 0.48567178524771226, 1}, 1e-15, PValueMethod.EXACT, false));
        builder.add(Arguments.of(
                new double[] {1, 2, 2, 5, 6},
                new double[] {0, 2, 2, 3, 3, 3},
                new double[] {0.4, 0.1, 0.4},
                new double[] {0.5946666666666665, 0.8594048608885093, 0.28348216705243917}, 1e-15, PValueMethod.EXACT, false));
        return builder.build();
    }
    ...
    static Stream<Arguments> testTwoSampleTwoSidedExactP() {
        ...
        builder.add(Arguments.of(516, 515, 13 * 515, false, -1, 0));
        builder.add(Arguments.of(516, 515, 13 * 515, true, -1, 0));
    ...
    @ParameterizedTest
    @CsvFileSource(resources = {"ks.twosample.twosided.medium.txt"}, delimiter = ' ')
    void testTwoSampleTwoSidedExactPMedium(int m, int n, long dmn, double p) {
        final int gcd = ArithmeticUtils.gcd(n, m);
        final double p2 = KolmogorovSmirnovTest.twoSampleExactP(dmn, n, m, gcd, false, true);
        TestUtils.assertProbability(p, p2, 5 * EPS, "two-sided");
    }

    @ParameterizedTest
    @CsvFileSource(resources = {"ks.twosample.twosided.large.txt"}, delimiter = ' ')
    void testTwoSampleTwoSidedExactPLarge(int m, int n, long dmn, double p) {
        final int gcd = ArithmeticUtils.gcd(n, m);
        final double p2 = KolmogorovSmirnovTest.twoSampleExactP(dmn, n, m, gcd, false, true);
        TestUtils.assertProbability(p, p2, EPS, "two-sided");
    }
    ...
    void testApprox(int n, int m) {
        ...
        double upperP = 0.1;
        int gcd = ArithmeticUtils.gcd(n, m);
        long z = findTwoSampleD(lowerP, n, m, 0, Long.MAX_VALUE, false, gcd);
        long z2 = findTwoSampleD(upperP, n, m, 0, z, true, gcd);
        ...
        for (long dnm = z2; dnm <= z; dnm += step) {
            double p = KolmogorovSmirnovTest.twoSampleExactP(dnm, n, m, gcd, false, true);
            ...
            double en = ((double) m * n) / ((double) m + n);
            double p1 = KolmogorovSmirnovDistribution.Two.sf(x, (int) Math.round(en));
            double e1 = Math.abs(p - p1) / Math.max(p, p1);
            x *= Math.sqrt(en);
            // From wikipedia:
            //x = x + 1.0 / (6 * Math.sqrt(en)) + (x - 1) / (4 * en);
            double p2 = KolmogorovSmirnovDistribution.ksSum(x);
            double e2 = Math.abs(p - p2) / Math.max(p, p2);
            if (e1 > maxe1) {
                maxe1 = e1;
                maxd1 = dnm;
            }
    ...
    void testTwoSampleProductSizeOverflow() {
        ...
        Assertions.assertFalse(Double.isNaN(KolmogorovSmirnovTest.test(x, y).getPValue()));
    }
    ...
    void testTwoSampleWithTies(double[] x, double[] y) {
        final TwoResult r = KolmogorovSmirnovTest.test(x, y);
        Arrays.sort(x);
        Arrays.sort(y);
        final int tiesX = (int) Arrays.stream(x).filter(v -> Arrays.binarySearch(y, v) >= 0).count();
        final int tiesY = (int) Arrays.stream(y).filter(v -> Arrays.binarySearch(x, v) >= 0).count();
        Assertions.assertEquals(tiesX, r.getTiesX(), "ties x");
        Assertions.assertEquals(tiesY, r.getTiesY(), "ties y");
        ...
        Assertions.assertEquals(r.getPValue(), KolmogorovSmirnovTest.test(y, x).getPValue(), "Sensitive to array input order");
    }
    ...
    void testTwoSampleWithManyTies() {
        ...
        TestUtils.assertRelativelyEquals(0.969490633213205, r.getPValue(), 1e-15, "p-value");
...
//        TestUtils.assertProbability(0.558364, p, 0.065, "p-value");
    ...
    private static void assertThrowsIllegalArgumentException(double[] x, double[] y) {
        Assertions.assertThrows(IllegalArgumentException.class,
                () -> KolmogorovSmirnovTest.statistic(x, y, AlternativeHypothesis.TWO_SIDED), "statistic");
        Assertions.assertThrows(IllegalArgumentException.class,
                () -> KolmogorovSmirnovTest.test(x, y), "test");
    }
    ...
    void testTwoSamplesAllEqual() {
        ...
        for (int i = 2; i < 30; i++) {
            ...
            Assertions.assertEquals(0.0, KolmogorovSmirnovTest.statistic(values, values, alternative), "statistic");
            Assertions.assertEquals(0.0, KolmogorovSmirnovTest.statistic(ascendingValues, ascendingValues, alternative), "statistic");
     ...
     * JIRA: MATH-1245
     *
     * <p>Verify that D-values are not viewed as distinct when they are mathematically equal
     * when computing p-statistics for small sample tests. Reference values are from R 3.2.0.
     *
     * <p>This test is no longer relevant as the public method to compute the p-value from
     * a d statistic (in [0, 1]) has been removed.
     */
    @Test
    // XXX move these cases to the two-sample test
    void testDRounding() {
        final double tol = 1e-12;
        final double[] x = {0, 2, 3, 4, 5, 6, 7, 8, 9, 12};
        final double[] y = {1, 10, 11, 13, 14, 15, 16, 17, 18};
        Assertions.assertEquals(0.0027495724090154106, KolmogorovSmirnovTest.test(x, y).getPValue(), tol);

        final double[] x1 = {2, 4, 6, 8, 9, 10, 11, 12, 13};
        final double[] y1 = {0, 1, 3, 5, 7};
        Assertions.assertEquals(0.085914085914085896, KolmogorovSmirnovTest.test(x1, y1).getPValue(), tol);

        final double[] x2 = {4, 6, 7, 8, 9, 10, 11};
        final double[] y2 = {0, 1, 2, 3, 5};
        Assertions.assertEquals(0.015151515151515027, KolmogorovSmirnovTest.test(x2, y2).getPValue(), tol);
    }
    ...
    void testTwoSampleEstimatePThrows() {
        ...
        final BiConsumer<double[], double[]> action =
            (x, y) -> KolmogorovSmirnovTest.estimateP(x, y, rng, 100, alternative, false);
        ...
        for (final int i : new int[] {-1, 0, Integer.MIN_VALUE}) {
            TestUtils.assertThrowsWithMessage(IllegalArgumentException.class,
                () -> KolmogorovSmirnovTest.estimateP(x, x, rng, i, alternative, false), Integer.toString(i));
        }
    }
    ...
    void testTwoSampleEstimatePSmallSamplesWithTies() {
        ...
        Assertions.assertEquals(0.0059, KolmogorovSmirnovTest.estimateP(x, y, rng, 10000, AlternativeHypothesis.TWO_SIDED, false), 1e-3);
    }
    ...
    void testTwoSampleEstimatePLargeSamples() {
        ...
        Assertions.assertEquals(0.0237, KolmogorovSmirnovTest.estimateP(GAUSSIAN, GAUSSIAN2, rng, 10000, AlternativeHypothesis.TWO_SIDED, true), 1e-2);
    }
    ...
    void testTwoSampleEstimatePRounding() {
        ...
        final UniformRandomProvider rng = RandomSource.XO_RO_SHI_RO_128_PP.create(1000);
        Assertions.assertEquals(0.06303, KolmogorovSmirnovTest.estimateP(x, y, rng, 10000, AlternativeHypothesis.TWO_SIDED, false), 1e-2);
    }
    ...
    void testMath1475() {
        ...
        final double p = KolmogorovSmirnovTest.test(x, y).getPValue();
