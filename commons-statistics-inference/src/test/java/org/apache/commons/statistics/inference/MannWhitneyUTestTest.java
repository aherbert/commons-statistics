...
import org.apache.commons.statistics.inference.MannWhitneyUTest.Options;
...
class MannWhitneyUTestTest {
    ...
    /** Options to tigger the wilcox distribution CDF(u). */
    private static final Options LESS_THAN = Options.builder().setAlternative(AlternativeHypothesis.LESS_THAN).build();
    ...
    void testOptions() {
        ...
        Assertions.assertEquals(true, o.isContinuityCorrection());
        ...
        for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
            for (final PValueMethod m : PValueMethod.values()) {
                for (final boolean correct : new boolean[] {true, false}) {
                    ...
                    Assertions.assertEquals(correct, o2.isContinuityCorrection());
                }
            }
        }
    }
    ...
    void testMannWhitneyUTestThrows() {
        assertMannWhitneyUTestThrows(MannWhitneyUTest::statistic);
        assertMannWhitneyUTestThrows(MannWhitneyUTest::test);
    }
    ...
    void testMannWhitneyU(double[] x, double[] y, double statistic, double[] p,
                          PValueMethod method, boolean correct, double eps) {
        Assertions.assertEquals(statistic, MannWhitneyUTest.statistic(x, y), "statistic");
        final Options.Builder b = Options.builder()
                                         .setPValueMethod(method)
                                         .setContinuityCorrection(correct);
        ...
        try {
            for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
                ...
                final MannWhitneyUTest.Result r =
                    ...
                    correct == Options.defaults().isContinuityCorrection() ?
                    MannWhitneyUTest.test(x, y) :
                    MannWhitneyUTest.test(x, y, b.build());
                ...
                Assertions.assertEquals(other, MannWhitneyUTest.statistic(y, x), () -> h + " statistic (y, x)");
            }
        } catch (final AssertionError e) {
            // print failing cases
            TestUtils.printf("x=c%s;%ny=c%s;%nwilcox.test(x, y, exact=%s, correct=%s, alternative='%c')%n",
    ...
    static Stream<Arguments> testMannWhitneyU() {
        ...
        builder.add(Arguments.of(
            ...
            PValueMethod.EXACT, ignoredContinuityCorrection, 0));
        builder.add(Arguments.of(
        ...
        builder.add(Arguments.of(
            ...
            new double[] {-3, -1, 1, 3, 5, 7, 9, 11},
            57, new double[] {0.046400658165364046, 0.023200329082682023, 0.98202385849444673},
            PValueMethod.AUTO, ignoredContinuityCorrection, 0));
        // Extreme u value
        builder.add(Arguments.of(
            new double[] {1, 2, 3, 4},
            new double[] {5, 6, 7, 8},
        ...
        builder.add(Arguments.of(
            ...
            PValueMethod.EXACT, ignoredContinuityCorrection, 32 * eps));
        builder.add(Arguments.of(
            ...
            PValueMethod.EXACT, ignoredContinuityCorrection, 16 * eps));
    ...
    void testBigDataSet() {
        ...
        final Options.Builder b = Options.builder();
        final double p = MannWhitneyUTest.test(x, y, b.setPValueMethod(PValueMethod.ASYMPTOTIC).build()).getPValue();
        ...
        Assertions.assertEquals(p, MannWhitneyUTest.test(x, y, b.setPValueMethod(PValueMethod.AUTO).build()).getPValue());
        Assertions.assertEquals(p, MannWhitneyUTest.test(x, y, b.setPValueMethod(PValueMethod.EXACT).build()).getPValue());
    }
    ...
    void testBigDataSetOverflow() {
        ...
        final double u = MannWhitneyUTest.statistic(x, y);
        Assertions.assertEquals(6.05e+09, u);
        final double result = MannWhitneyUTest.test(x, y).getPValue();
    ...
    void testCalculateExactPValueTooLarge() {
        ...
        Assertions.assertEquals(-1, MannWhitneyUTest.calculateExactPValue(1L << 32, m, n, Options.defaults()));
    }
    ...
    void testCDF(int u, int m, int n, double p) {
        ...
        TestUtils.assertProbability(p, MannWhitneyUTest.calculateExactPValue(u, m, n, LESS_THAN), 1e-14, "p-value");
    }
