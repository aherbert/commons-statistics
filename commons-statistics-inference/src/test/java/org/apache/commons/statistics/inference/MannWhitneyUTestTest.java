...
class MannWhitneyUTestTest {
        ...
        for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
            for (final PValueMethod m : PValueMethod.values()) {
                for (final boolean correct : new boolean[] {true, false}) {
                    for (final double mu  : new double[] {-1, 0, 1}) {
                        final Options o1 = o.toBuilder().setAlternative(h).setPValueMethod(m)
                            .setContinuityCorrection(correct).setMu(mu).build();
                        ...
                        Assertions.assertEquals(mu, o1.getMu());
                        ...
                        Assertions.assertEquals(mu, o2.getMu());
                    }
                }
            }
        }
    ...
    void testMannWhitneyUTestThrows() {
        assertMannWhitneyUTestThrows((x, y) -> MannWhitneyUTest.statistic(0, x, y));
    ...
    void testMannWhitneyU(double[] x, double[] y, double statistic, double[] p,
        ...
        Assertions.assertEquals(statistic, MannWhitneyUTest.statistic(mu, x, y), "statistic");
        final Options.Builder b = Options.builder()
                                         .setMu(mu)
        ...
        try {
            for (final AlternativeHypothesis h : AlternativeHypothesis.values()) {
                ...
                Assertions.assertEquals(other, MannWhitneyUTest.statistic(-mu, y, x), () -> h + " statistic (y, x)");
            }
    ...
    void testBigDataSetOverflow() {
        ...
        final double u = MannWhitneyUTest.statistic(0, x, y);
