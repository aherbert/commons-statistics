...
final class StatisticUtils {
    ...
    static double computeRatio(double[] expected, long[] observed) {
        InferenceUtils.checkValuesRequiredSize(expected.length, 2);
        InferenceUtils.checkValuesSizeMatch(expected.length, observed.length);
        InferenceUtils.checkStrictlyPositive(expected);
