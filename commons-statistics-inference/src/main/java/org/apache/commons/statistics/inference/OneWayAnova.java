...
public final class OneWayAnova {
     ...
     */
    public static Result test(Collection<double[]> data) {
        InferenceUtils.checkCategoriesRequiredSize(data.size(), 2);
        long n = 0;
        for (final double[] array : data) {
            n += array.length;
            InferenceUtils.checkValuesRequiredSize(array.length, 1);
        }
