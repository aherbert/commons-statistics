...
final class KolmogorovSmirnovDistribution {
    ...
    static final class Two {
        ...
        static double sf(double x, int n) {
            ...
            // nx^1.5 <= 1.4
            if (n <= 100000 && n * Math.pow(x, 1.5) < 1.4) {
                // Durbin matrix (MTW)
                return 1 - durbinMTW(x, n);
            }
        ...
        private static double sfExact(double x, int n) {
            ...
            if (nx <= 1) {
                ...
                // XXX set switch threshold where (2x - 1/n)^n is sub-normal
                // Max threshold is n=170
                if (n < 20) {
                    return 1 - Factorial.doubleValue(n) * Math.pow(f, n);
                }
                return -Math.expm1(LogFactorial.create().value(n) + n * Math.log(f));
            }
            // 1 - 1/n <= x < 1
            if (n - 1 <= nx) {
                // 2 * (1-x)^n
    ...
    static final class One {
        ...
        static double sf(double x, int n) {
            ...
            // XXX Choice of algorithm could be optional
